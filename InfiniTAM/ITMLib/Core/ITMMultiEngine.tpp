...
ITMTrackingState::TrackingResult ITMMultiEngine<TVoxel, TIndex>::ProcessFrame(ITMUChar4Image *rgbImage, ITMShortImage *rawDepthImage, ITMIMUMeasurement *imuMeasurement)
{
	...
	for (size_t i = 0; i < todoList.size(); ++i)
	{
		...
		if (todoList[i].fusion) denseMapper->ProcessFrame(view, currentLocalMap->trackingState, currentLocalMap->scene, *(new std::unordered_set< Vector3i, ORUtils::MyHashFunction >), *(new std::vector< Vector3i >), currentLocalMap->renderState);
